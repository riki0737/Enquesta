...
class EnquestesVisitor(ParseTreeVisitor):
    ...
    def visitItem(self, ctx:EnquestesParser.ItemContext):
        ...
        n = ctx.getChildCount()
        l = [next(g) for i in range(n)]
        self.AST.add_edge(l[3].getText(),l[5].getText(), item=l[0].getText(), color = 'green', textItem=l[0].getText())
    ...
    def visitAlternative(self, ctx:EnquestesParser.AlternativeContext):
        ...
        for k in range(5,n-1):
            if l[j].getText() != ',':
                addid = self.visit(l[j])
                self.AST.add_edge(fixid,addid,color='blue')
